...
class SensoryToolkit extends EventEmitter {
  ...
  /**
   * Get regulation tools for specific arousal level (compatibility method)
   */
  getToolsForArousal(heroId, arousalLevel) {
    const profile = this.profiles.get(heroId);
    if (!profile) return [];

    // Update arousal state if provided
    if (arousalLevel) {
      profile.updateState(arousalLevel);
    }

    // Use getSuggestions with current arousal
    return this.getSuggestions(heroId, { limit: 5 });
  }
...
module.exports.default = SensoryToolkit;