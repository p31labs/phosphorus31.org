...
export default function CognitiveHubDemo() {
  ...
  const addMessage = useCallback(
    (content: string, category: HubMessage['category'] = 'thought') => {
  ...
  const handleQuickInput = useCallback(
    (input: string) => {
      const category: HubMessage['category'] = input.includes('TODO')
        ? 'task'
        : input.includes('meeting') || input.includes('notes')
          ? 'note'
          : input.includes('?')
            ? 'question'
            : 'thought';
  ...
  const handleCustomInput = useCallback(() => {
    ...
    if (content) {
      const category: HubMessage['category'] =
        content.includes('TODO') || content.includes('need to')
          ? 'task'
          : content.includes('meeting') || content.includes('notes')
            ? 'note'
            : content.includes('?')
              ? 'question'
              : 'thought';
