...
export default function QuantumEntanglementDemo() {
  ...
  // Measure particle
  const measureParticle = useCallback(
    async (particleId: string) => {
      if (!sessionId || isMeasuring) return;

      setIsMeasuring(true);
      try {
        const measurement = await quantumEntanglementBridge.measureEntangledParticle(
          sessionId,
          particleId
        );
          ...
          if (particle) {
            ...
            particle.entanglementLines.forEach((line) => {
              ...
              if (correlatedParticle) {
                correlatedParticle.glowIntensity = Math.max(
                  correlatedParticle.glowIntensity,
                  line.strength * 0.8
                );
  ...
  const animate = useCallback(() => {
    ...
    particles.forEach((particle) => {
      particle.entanglementLines.forEach((line) => {
        ...
        if (target) {
          const alpha =
            line.strength * (0.3 + Math.sin(Date.now() * 0.003 + particle.animationPhase) * 0.2);
    ...
    particles.forEach((particle, id) => {
      ...
      ctx.fillText(particle.id.replace('particle_', ''), particle.x, particle.y + 35);

      // Measurement value
      if (particle.isMeasured) {
        ctx.fillStyle = 'rgba(6, 182, 212, 0.8)';
        ctx.font = '10px monospace';
        ctx.fillText(particle.state.measurement.toFixed(3), particle.x, particle.y + 48);
      }
    ...
    const handleEntanglementPropagated = (data: any) => {
      ...
      setParticles((prev) => {
        ...
        if (targetParticle) {
          ...
          targetParticle.glowIntensity = Math.max(
            targetParticle.glowIntensity,
  ...
  return (
    ...
    >
      ...
      {showInstructions && (
        ...
        >
          ...
          <div style={{ fontSize: '13px', lineHeight: '1.6' }}>
            <p>
              <strong>Step 1:</strong> Create an entanglement session below
            </p>
            <p>
              <strong>Step 2:</strong> Click particles to measure them
            </p>
            <p>
              <strong>Step 3:</strong> Watch as entangled particles correlate instantly!
            </p>
            <p>
              <strong>Step 4:</strong> Open multiple tabs for true quantum entanglement across
              browsers
            </p>
            <p style={{ color: '#06b6d4', fontStyle: 'italic' }}>
              "When you measure one entangled particle, the other instantly knows - even across
          ...
          >
            {measurementHistory.slice(-5).map((entry, i) => (
              <div key={i} style={{ marginBottom: '4px' }}>
                ...
                <span style={{ color: '#06b6d4' }}> P{entry.particleId.split('_')[1]}</span>
                <span style={{ color: '#10b981' }}> → {entry.measurement.toFixed(4)}</span>
      ...
      >
        ...
        <div style={{ fontSize: '13px', lineHeight: '1.6' }}>
          <p>
            <strong>For maximum magic:</strong> Open multiple browser tabs with this demo running.
          </p>
          <p>
            Each tab will synchronize quantum states in real-time using WebRTC peer-to-peer
            connections.
          </p>
          <p>
            Measuring a particle in one tab instantly updates its entangled partners in other tabs!
          </p>
          <p style={{ color: '#8b5cf6', fontStyle: 'italic', marginTop: '8px' }}>
            "This is as close as classical computers can get to Einstein's 'spooky action at a
            distance' ✨"
          </p>
        </div>
      </div>
