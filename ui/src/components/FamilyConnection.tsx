...
export default function FamilyConnection() {
  ...
  const toggleMemberStatus = useCallback((id: string) => {
    setFamilyStatus((prev) =>
      prev.map((m) =>
        m.id === id ? { ...m, status: m.status === 'present' ? 'away' : 'present' } : m
      )
    );
  }, []);
          ...
          return (
            <div
              ...
              style={{
                ...
                boxShadow: member.status === 'present' ? `0 0 20px ${member.color}44` : 'none',
          ...
          transform:
            breathPhase === 'inhale'
              ...
              : breathPhase === 'hold'
                ...
                : 'scale(0.9)',
          border: `2px solid ${
            breathPhase === 'inhale' ? '#3498db' : breathPhase === 'hold' ? '#9b59b6' : '#2ecc71'
          }44`,
        }}
      >
        <div
