...
import { OrbitControls, Text, Html, Sphere, Line, Torus, Box } from '@react-three/drei';
...
export default function PhenixNavigatorDemo() {
  ...
  const [demoMode, setDemoMode] = useState<'prepare' | 'channel' | 'tomography' | 'autopoiesis'>(
    'prepare'
  );
        ...
        >
          ...
          <div
            style={{
              ...
              border: `1px solid ${systemState.status === 'SECURE' ? '#00ff00' : systemState.status === 'CAUTION' ? '#ffff00' : '#ff0000'}`,
              borderRadius: '8px',
              padding: '15px',
              minWidth: '200px',
            }}
          ...
          >
            ...
            <div style={{ marginBottom: '10px' }}>Fairness: |⟨ψᵢ|ψⱼ⟩|² = 1/3 for all i ≠ j</div>
            <div
              style={{ fontStyle: 'italic', borderTop: '1px solid #00ffff', paddingTop: '10px' }}
            >
              "The tetrahedron is the minimum structural system in three-dimensional space. It
              cannot be deformed without breaking. This is why Phenix uses it — security based on
              geometry, not computational complexity."
            </div>
          </div>
        </div>
      </div>
        ...
        >
          {[
            ...
            <button
              ...
              style={{
                padding: '10px 20px',
                background: demoMode === mode.id ? '#00ffff' : 'rgba(0, 10, 20, 0.8)',
                color: demoMode === mode.id ? '#000' : '#00ffff',
                border: '1px solid #00ffff',
                borderRadius: '5px',
                cursor: 'pointer',
                fontFamily: 'monospace',
          ...
          >
            {[0, 1, 2, 3].map((i) => (
              <button
                ...
                style={{
                  padding: '15px',
                  background: selectedState === i ? '#00ff00' : 'rgba(0, 10, 20, 0.8)',
                  color: '#00ffff',
                  border: '1px solid #00ffff',
                  borderRadius: '50%',
                  cursor: 'pointer',
        ...
        {demoMode === 'autopoiesis' && (
          ...
          >
            <button
              ...
              style={{
                padding: '15px 30px',
                background: systemState.healing ? '#ffff00' : '#00ff00',
...
function BlochSphere() {
  return (
    <group>
      ...
      <Text position={[0, 0, 1.2]} fontSize={0.1} color="#ffffff" anchorX="center" anchorY="middle">
        |0⟩
      </Text>
  ...
  return (
    <group ref={groupRef}>
      ...
      {TETRAHEDRON_EDGES.map(([i, j], index) => (
        <Line key={index} points={[vectors[i], vectors[j]]} color="#00ffff" lineWidth={3} />
      ))}
    ...
    if (meshRef.current) {
      if (selected) {
        ...
        material.emissive.setRGB(
          0,
          Math.sin(state.clock.elapsedTime * 4) * 0.5 + 0.5,
          Math.sin(state.clock.elapsedTime * 4) * 0.5 + 0.5
        );
      } else if (healing) {
  ...
  return (
    <group position={position}>
      <Sphere ref={meshRef} args={[0.08, 16, 16]}>
        <meshBasicMaterial color={selected ? '#00ff00' : '#00ffff'} emissive="#000000" />
      </Sphere>
      ...
      {selected && (
        <Line points={[new THREE.Vector3(0, 0, 0), position]} color="#00ff00" lineWidth={2} />
      )}
    </group>
  );
}
