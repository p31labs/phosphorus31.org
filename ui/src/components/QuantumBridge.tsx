...
function TetrahedronEdges({ color, intensity }: { color: string; intensity: number }) {
  ...
  return (
    <lineSegments ref={linesRef}>
      <bufferGeometry />
      <lineBasicMaterial vertexColors transparent opacity={intensity} linewidth={2} />
    </lineSegments>
  );
}
  ...
  return (
    ...
    >
        ...
        <div className="absolute top-4 right-4 z-10 text-right space-y-1">
          ...
          {touch.active && (
            <div
              ...
              style={{
                color: K4_COLORS[Math.floor(touch.y / 160) * 2 + Math.floor(touch.x / 240)],
              }}
            >
      ...
      <Canvas camera={{ position: [3, 3, 3], fov: 50 }}>
        ...
        <BlochSphere />
        <QuantumTetrahedron accel={accel} touch={touch} mode={mode} battery={battery} />
