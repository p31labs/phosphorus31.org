...
function SpoonCostIndicator({ cost, currentSpoons }: { cost: number; currentSpoons: number }) {
  ...
  return (
    ...
    >
      ...
      {!canAfford && <span style={{ color: '#FCA5A5', fontWeight: 600 }}>⚠️</span>}
    </div>
  );
}
...
function RevealButton({ voltage, onReveal, revealed }: RevealButtonProps) {
  ...
  const buttonColor = voltage >= 7 ? (confirmStep === 0 ? '#B91C1C' : '#DC2626') : '#4B5563';
...
export function MessageDisplay({
  ...
  return (
    ...
    >
      ...
      >
        <span style={{ fontWeight: 500, color: '#F3F4F6' }}>From: {raw.sender}</span>
      ...
      <div style={{ padding: '16px' }}>
        ...
        <section>
          ...
          >
            ...
            <RevealButton voltage={voltage.score} onReveal={handleReveal} revealed={localShowRaw} />
          </div>
