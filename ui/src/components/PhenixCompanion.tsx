...
export default function PhenixCompanion({
  isOpen,
  onClose,
  initialMode = 'ARCHITECT',
}: PhenixCompanionProps) {
  ...
  const { processedPayload } = useShieldStore();

  const { spoons, heartbeat } = operator;
  const processed = processedPayload ? [processedPayload] : [];

  // Calculate board state based on spoons and processed messages
  const calculateBoardState = (): BoardState => {
    if (spoons >= 80) return 'GREEN'; // Default healthy threshold
    if (spoons >= 40) return 'YELLOW'; // Default caution threshold
    return 'RED';
  };
      ...
      >
        ...
        <div style={{ display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          >
            ...
            <span style={{ color: GOD_CONFIG.theme.text.secondary }}>{spoons} spoons</span>
          </div>
          ...
          >
            <div
              style={{
                ...
                borderRadius: 12,
                color: message.role === 'user' ? '#fff' : GOD_CONFIG.theme.text.primary,
                fontSize: 14,
                lineHeight: 1.4,
                whiteSpace: 'pre-wrap',
              }}
