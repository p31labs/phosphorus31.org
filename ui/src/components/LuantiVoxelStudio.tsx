...
export default function LuantiVoxelStudio() {
  ...
  useEffect(() => {
    ...
    const handleWorldUpdate = (data: any) => {
      ...
      setWorldStats({
        ...
        averageCoherence:
          Array.from(data.worldData.players.values()).reduce(
            (sum: number, p: any) => sum + p.coherence,
            0
          ) / data.worldData.players.size,
        totalBlocksPlaced: Array.from(data.worldData.players.values()).reduce(
          (sum: number, p: any) => sum + p.blocksPlaced,
          0
        ),
      });
    ...
    const handleEntangledBlockPlaced = (data: any) => {
      console.log(
        `🔗 Entangled blocks created: ${JSON.stringify(data.position)} ↔ ${JSON.stringify(data.entangledPosition)}`
      );
    };
  ...
  const handleVoxelClick = useCallback(
    (position: [number, number, number], isRightClick: boolean) => {
      ...
      if (isRightClick) {
        ...
        luantiWorldEngine.updatePlayerQuantumState('Player', coherence, {
          x: x + 5,
          y: y + 2,
          z: z + 5,
        });
      ...
      >
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontSize: '18px', marginBottom: '10px' }}>
            Initializing Luanti Voxel Studio
          </div>
    ...
    >
      ...
      >
        ...
        >
          ...
          {!isWorldRunning ? (
            ...
            <div style={{ fontSize: '12px', color: '#22c55e' }}>✅ Quantum World Active</div>
          )}
        </div>
          ...
          {[
            ...
            <button
              ...
              style={{
                ...
                border: 'none',
                borderBottom:
                  activeTab === tab.id ? `2px solid ${GOD_CONFIG.theme.text.accent}` : 'none',
                color:
                  activeTab === tab.id
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px' }}>
                ...
                Array.from(players.entries()).map(([name, player]) => (
                  ...
                  >
                    ...
                    <div style={{ fontSize: '11px', color: GOD_CONFIG.theme.text.secondary }}>
                      Coherence: {(player.coherence * 100).toFixed(1)}% | Blocks:{' '}
                      {player.blocksPlaced}
                    </div>
                    <div
                      style={{
          ...
          {activeTab === 'quantum' && (
            <div>
              ...
              <div style={{ fontSize: '12px', lineHeight: '1.6' }}>
                <div>
                  🌌 <strong>Coherence:</strong> Player actions affect quantum stability
                </div>
                <div>
                  🔗 <strong>Entanglement:</strong> Linked blocks across space
                </div>
                <div>
              ...
              {selectedPlayer && players.has(selectedPlayer) && (
                <div style={{ marginTop: '16px' }}>
                  ...
                  <div style={{ fontSize: '11px' }}>
                    ...
                    <div>
                      Coherence: {(players.get(selectedPlayer)?.coherence || 0 * 100).toFixed(1)}%
                    </div>
          ...
          {activeTab === 'blocks' && (
            <div>
              ...
              >
                ...
                <br />• High coherence = quantum effects
              </div>
            </div>
          )}
        </div>
      ...
      <div style={{ flex: 1, position: 'relative' }}>
        ...
        {isPlacing && (
          ...
          >
            🏗️ Placing{' '}
            {VOXEL_TYPES[selectedVoxelType as keyof typeof VOXEL_TYPES]?.name || selectedVoxelType}
            ...
          </div>
        )}
...
function VoxelWorld({
  ...
  return (
    <group>
      ...
      {/* Quantum effects */}
      {showQuantumEffects && <QuantumEffects entangledBlocks={entangledBlocks} />}
    </group>
  );
}

// Quantum Player Component
function QuantumPlayer({
  name,
  position,
  coherence,
}: {
