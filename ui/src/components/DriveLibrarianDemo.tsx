...
export default function DriveLibrarianDemo() {
  ...
  const handleProcessFile = useCallback(async () => {
    ...
    try {
      ...
      if (
        selectedFile.content.includes('meta-analysis') ||
        selectedFile.content.includes('peer-reviewed')
      ) {
      ...
      if (
        selectedFile.content.includes('randomized controlled trial') ||
        selectedFile.content.includes('double-blind')
      ) {
        mockResult.claimsFound.push({
      ...
      if (
        selectedFile.content.includes('hypothesis') ||
        selectedFile.content.includes('further research')
      ) {
        mockResult.claimsFound.push({
      ...
      if (
        selectedFile.content.includes('we believe') ||
        selectedFile.content.includes('could be faster')
      ) {
        mockResult.claimsFound.push({
      ...
      // Determine category
      if (
        selectedFile.name.includes('Research') ||
        selectedFile.content.includes('study') ||
        selectedFile.content.includes('trial')
      ) {
        mockResult.category = 'Research';
      } else if (
        selectedFile.name.includes('Legal') ||
        selectedFile.content.includes('agreement') ||
        selectedFile.content.includes('contract')
      ) {
        mockResult.category = 'Legal';
      } else if (
        selectedFile.name.includes('Notes') ||
        selectedFile.content.includes('meeting') ||
        selectedFile.content.includes('discussion')
      ) {
    ...
    if (!showPII) {
      ...
      for (const pii of result.piiFound) {
        redactedContent = redactedContent.replace(
          pii.text,
          pii.redactedText || `[${pii.entityType}-REDACTED]`
        );
      }
  ...
  const getEvidenceIcon = (level: string) => {
    switch (level) {
      ...
      case 'speculative':
        return <AlertTriangle size={16} className="text-pink-600" />;
      default:
  ...
  return (
    ...
    >
      ...
      <div style={{ marginBottom: '2rem' }}>
        ...
        >
          Advanced PII detection, evidence categorization, and document organization. Protect
          sensitive information while extracting scientific insights.
        </p>
      </div>
        ...
        >
          {SAMPLE_FILES.map((file) => (
            ...
            >
              ...
              >
                {file.type.charAt(0).toUpperCase() + file.type.slice(1)} • {file.content.length}{' '}
                chars
              </p>
            </div>
          ))}
                ...
                {processingResults.piiFound.map((pii, index) => (
                  <div
                    ...
                    style={{
                      ...
                      padding: '0.5rem 0',
                      borderBottom:
                        index < processingResults.piiFound.length - 1
              ...
              {batchResults.results.map((result, index) => (
                ...
                >
                  ...
                  <div style={{ display: 'flex', gap: '1rem', fontSize: '0.875rem' }}>
                    <span style={{ color: '#ef4444' }}>{result.piiFound.length} PII</span>
                    <span style={{ color: '#8b5cf6' }}>{result.claimsFound.length} claims</span>
                    <span style={{ color: '#22c55e' }}>{result.category}</span>
                  </div>
                </div>
              ))}
            </div>
      ...
      >
        ...
        >
          🔒 <strong>Privacy First:</strong> All PII detection happens locally. No data leaves your
          device.
          <br />
          🧠 <strong>Evidence-Based:</strong> Scientific claims are categorized by methodological
          rigor.
          <br />
          📚 <strong>Organized Knowledge:</strong> Documents are automatically categorized and
          highlighted.
        </p>
      </div>
    </div>
