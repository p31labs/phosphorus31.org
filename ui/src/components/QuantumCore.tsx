      ...
      {K4_NODES.map((node, i) => (
        <Float key={node.id} speed={2} rotationIntensity={0.5} floatIntensity={0.3}>
          <group position={TETRA_VERTICES[i].clone().multiplyScalar(2).toArray()}>
            ...
            {/* Energy aura */}
            {activeNode === i && (
              <Sparkles count={50} scale={1} size={3} speed={0.5} color={node.color} />
            )}
          </group>
        </Float>
      ))}
...
function GoldenSpiral({ scale = 1 }) {
  ...
  const geometry = useMemo(() => {
    ...
    for (let i = 0; i < turns * pointsPerTurn; i++) {
      ...
      points.push(new THREE.Vector3(r * Math.cos(theta), t * 0.1 - 0.3, r * Math.sin(theta)));
    }
...
function MeshNetwork({ nodeCount = 12, connected = false }) {
  ...
  const nodes = useMemo(() => {
    ...
    for (let i = 0; i < nodeCount; i++) {
      ...
      const pos = TETRA_VERTICES[vertexIndex].clone().multiplyScalar(1.5).add(groupOffset);
    ...
    <group ref={groupRef} position={[0, 0, -5]}>
      {nodes.map((pos, i) => (
        <group key={i} position={pos.toArray()}>
          <Trail width={0.5} length={5} color={K4_NODES[i % 4].color} attenuation={(t) => t * t}>
            <mesh>
  ...
  return (
    <div className="absolute inset-0 pointer-events-none font-mono text-xs">
      ...
      <div className="absolute bottom-4 right-4 text-right">
        <div
          className={`flex items-center gap-2 ${meshConnected ? 'text-green-400' : 'text-zinc-500'}`}
        >
          <span
            className={`w-2 h-2 rounded-full ${meshConnected ? 'bg-green-400 animate-pulse' : 'bg-zinc-600'}`}
          />
          MESH: {meshConnected ? 'CONNECTED' : 'OFFLINE'}
        </div>
        <div className="text-amber-400/60 text-[10px] mt-1">φ = {PHI.toFixed(10)}</div>
      </div>
...
function AccelerometerCamera({
  ...
  useFrame(() => {
    ...
    camera.position.lerp(new THREE.Vector3(targetRef.current.x, targetRef.current.y, 8), 0.05);
    camera.lookAt(0, 0, 0);
  });
    ...
    <group ref={groupRef} position={[0, 4, 0]}>
      {[...Array(ringCount)].map((_, i) => (
        <mesh key={i} rotation={[Math.PI / 2, 0, 0]} position={[0, -i * 0.3, 0]}>
          ...
          <meshBasicMaterial color="#00ff88" transparent opacity={0.5 - i * 0.1} />
        </mesh>
      ))}
    </group>
  );
...
export default function QuantumCore() {
  ...
  return (
    <div className="relative w-full h-[600px] bg-black rounded-xl overflow-hidden border border-zinc-800">
      ...
      <HUD voltage={voltage} spoons={spoons} meshConnected={isConnected} />
        ...
        <div className="absolute bottom-4 left-4 bg-black/80 rounded-lg p-3 border border-cyan-500/30 font-mono text-xs">
          ...
          <button
            ...
            className={`mt-2 w-full py-1 rounded text-xs ${
              useDeviceCamera ? 'bg-cyan-500/30 text-cyan-400' : 'bg-zinc-700/50 text-zinc-500'
            }`}
          >
