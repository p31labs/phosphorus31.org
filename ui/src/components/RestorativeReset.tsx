...
const RESET_PHASES: readonly ResetPhase[] = [
  {
    ...
    somatic:
      'Progressive muscle relaxation: Tense each muscle group for 5 seconds, release for 10 seconds. Start with feet, move to head.',
    validation:
      'You are not broken. You are in metabolic debt. This is hardware, not character. The Shield is active.',
  },
  {
    id: 'short-term',
    ...
    validation:
      'You have permission to stop performing. Rest is a maintenance requirement, not a failure.',
  },
  {
    id: 'medium-term',
    ...
    somatic:
      'Daily 4-4-8 breathing sessions: Morning, afternoon, evening. Minimum 3 cycles per session.',
    validation:
      'The past cannot hurt you right now. The future has not happened. In this moment, you are safe.',
  },
  {
    id: 'long-term',
    ...
    somatic:
      'Integrate somatic grounding into daily routine. Use as preventive measure, not just recovery.',
    validation:
      'You are a non-linear transducer navigating a high-voltage world. The Shield is your prosthetic cortex.',
  },
] as const;
    ...
    return (
      ...
      >
        ...
        <p style={{ margin: 0, fontSize: 13, color: GOD_CONFIG.theme.text.primary }}>
          Your status is above the critical threshold. The Restorative Reset protocol is not
          required at this time. Continue with standard maintenance protocols.
        </p>
      </div>
    );
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      >
        ...
        <p style={{ margin: 0, fontSize: 13, color: GOD_CONFIG.theme.text.secondary }}>
          Neurodivergent burnout recovery protocol for Critical status events. This protocol is
          designed to restore metabolic integrity and prevent Topological Arrest.
        </p>
      </div>
      ...
      {isCritical && (
        ...
        >
          ...
          <div style={{ fontSize: 12, color: GOD_CONFIG.theme.text.primary, lineHeight: 1.6 }}>
            Your status is below 25%. The Restorative Reset protocol is now active. Follow the
            phases below to restore metabolic integrity and return to Green Board status.
          </div>
        </div>
      )}
      ...
      {deepProcessingQueue.length > 0 && (
        ...
        >
          ...
          <div style={{ fontSize: 12, color: GOD_CONFIG.theme.text.primary, lineHeight: 1.6 }}>
            {deepProcessingQueue.length} message{deepProcessingQueue.length !== 1 ? 's' : ''} are
            currently gated. These will be available for processing when your status improves to ≥
            40%. Focus on recovery first.
          </div>
        </div>
      )}
      ...
      {completedPhases.size > 0 && (
        ...
        >
          ...
          <div style={{ fontSize: 12, color: GOD_CONFIG.theme.text.primary, lineHeight: 1.6 }}>
            {completedPhases.size} of {RESET_PHASES.length} phases completed. Continue following the
            protocol to restore Green Board status.
          </div>
        </div>
      )}
    </div>
