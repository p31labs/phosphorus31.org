...
import {
  ...
  Text,
  Sphere,
  Tetrahedron as DreiTetrahedron,
  Line,
...
export default function CoherenceQuest() {
  ...
  useEffect(() => {
    const gameLoop = setInterval(() => {
      ...
      const newScore =
        gameState.score + (coherenceBonus + levelBonus + moleculeBonus) * gameState.multiplier;
      ...
      const gameOver =
        entropy > 0.8 - gameState.level * 0.05 || gameState.time > 300 + gameState.level * 60;
      ...
      if (Math.abs(trimtabInput.value - 0.5) > 0.1) {
        fisherEscolaEngine.applyPerturbation(
          'field',
          (trimtabInput.value - 0.5) * 0.001 * gameState.level
        );
      }
  ...
  const getAchievementIcon = (achievement: string) => {
    switch (achievement) {
      case 'coherence_master':
        return '🎯';
      case 'molecule_collector':
        return '🧬';
      case 'quantum_survivor':
        return '⚡';
      case 'multiplier_mage':
        return '✨';
      default:
        return '🏆';
    }
  ...
  const getAchievementName = (achievement: string) => {
    switch (achievement) {
      case 'coherence_master':
        return 'Coherence Master (Entropy < 10%)';
      case 'molecule_collector':
        return 'Molecule Collector (20+ molecules)';
      case 'quantum_survivor':
        return 'Quantum Survivor (Level 3+)';
      case 'multiplier_mage':
        return 'Multiplier Mage (3x+ bonus)';
      default:
        return achievement;
    }
  };
  ...
  return (
    ...
    >
      ...
      <Canvas camera={{ position: [0, 0, 5], fov: 75 }}>
        ...
        <group position={[0, 0, -2]}>{getCoherenceVisualization()}</group>
      ...
      >
        ...
        <div style={{ marginTop: '10px' }}>
          ...
          <div style={{ fontSize: '11px', marginTop: '5px', color: '#888' }}>
            ...
            {keyboardMode && (
              <span style={{ color: '#22c55e' }}>
                <br />
                🎮 A/D: Coarse adjust | W: Fine tune | S: Center
              </span>
            )}
          </div>
          ...
          <div style={{ fontSize: '10px', marginTop: '5px', color: '#666' }}>
            Velocity: {trimtabInput.velocity.toFixed(3)} | Acceleration:{' '}
            {trimtabInput.acceleration.toFixed(3)}
          </div>
        </div>
          ...
          <div style={{ marginTop: '20px', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: '12px', marginBottom: '15px' }}>
              ...
              {gameState.score > gameState.highScore && (
                <div style={{ color: '#22c55e', fontWeight: 'bold' }}>🎉 NEW HIGH SCORE!</div>
              )}
            </div>
            <button
              onClick={resetGame}
      ...
      >
        ...
        >
          <div>
            ...
            <br />
            • Trimtab: Gentle quantum field adjustment
            <br />
            • Center (0.5) = equilibrium
            <br />• Too far = entropy increase
          </div>

          <div>
            <strong>🏆 Scoring:</strong>
            <br />
            • Coherence bonus: Distance from target
            <br />
            • Level multiplier: Increases difficulty
            <br />
            • Molecule bonus: +0.5 pts each
            <br />• Perfect streaks: Multiplier boost
          </div>
        </div>
        ...
        <div style={{ textAlign: 'center', fontSize: '11px', color: '#888' }}>
          🎹 Enable keyboard mode for precise control | 🏆 Unlock achievements | ⚡ Higher levels =
          more molecules
        </div>
      </div>
    </div>
...
function GameWorld({
  ...
  return (
    <group ref={groupRef}>
      {/* Dynamic lighting */}
      <pointLight ref={lightRef} position={[5, 5, 5]} intensity={0.4} distance={50} decay={2} />
      ...
      {molecules.slice(0, 50).map((molecule) => (
        <PosnerMolecule
          key={molecule.id}
          position={[
            molecule.position.x * 0.08,
            molecule.position.y * 0.08,
            molecule.position.z * 0.08,
          ]}
      ...
      {coherence > 0.6 && <CoherenceWave center={[0, 0, 0]} radius={15} coherence={coherence} />}
    </group>
  );
}

// Quantum field particle
function QuantumFieldParticle({
  position,
  intensity,
  entropy,
  coherence,
    ...
    <group ref={groupRef} position={position}>
      {Array.from({ length: 3 }, (_, i) => (
        <mesh key={`vortex-ring-${i}`} rotation={[Math.PI / 2, 0, (i * Math.PI) / 3]}>
          ...
          <meshBasicMaterial color="#ef4444" transparent opacity={intensity * 0.4} />
        </mesh>
      ))}
    </group>
  );
    ...
    <group ref={groupRef} position={position}>
      ...
      {calciumPositions.map((pos, i) => (
        <mesh
          key={`ca-${i}`}
          position={[
            pos[0] * moleculeSize * 2,
            pos[1] * moleculeSize * 2,
            pos[2] * moleculeSize * 2,
          ]}
        >
      ...
      {phosphatePositions.map((pos, i) => (
        <mesh
          key={`po4-${i}`}
          position={[
            pos[0] * moleculeSize * 3,
            pos[1] * moleculeSize * 3,
            pos[2] * moleculeSize * 3,
          ]}
        >
  ...
  return (
    <group ref={groupRef}>
      ...
      {Array.from({ length: 8 }, (_, i) => (
        <mesh key={i} position={[Math.sin(i) * 0.8, Math.cos(i) * 0.8, Math.sin(i * 2) * 0.8]}>
  ...
  useEffect(() => {
    ...
    molecules.forEach((molecule) => {
      ...
      if (x >= 0 && x < 120 && y >= 0 && y < 80) {
        // Molecule dot
        ctx.fillStyle =
          molecule.coherence > 0.7 ? '#22c55e' : molecule.coherence > 0.4 ? '#eab308' : '#ef4444';
        ctx.beginPath();
