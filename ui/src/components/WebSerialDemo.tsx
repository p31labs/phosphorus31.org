...
export default function WebSerialDemo() {
  ...
  const [connectionStatus, setConnectionStatus] = useState<
    'disconnected' | 'connecting' | 'connected' | 'secure'
  >('disconnected');
  ...
  return (
    ...
    >
      ...
      >
        ...
        />
        <span style={{ fontWeight: '500' }}>{getConnectionStatusText()}</span>
        {devices.length > 0 && (
          ...
          >
            {devices.map((device) => (
              <option key={device.deviceId} value={device.deviceId}>
                {device.deviceId} ({device.status}) - Last seen:{' '}
      ...
      <details style={{ marginTop: '20px' }}>
        ...
        >
          <div>
            <strong>Web Serial API:</strong> Direct USB communication with ESP32
          </div>
          <div>
            <strong>Protocol:</strong> Custom binary protocol with checksums
          </div>
          <div>
            <strong>Security:</strong> ML-KEM + X25519 hybrid encryption
          </div>
          <div>
            <strong>Baud Rate:</strong> 115200 bps
          </div>
          <div>
            <strong>ESP32 Support:</strong> S3, C3, and other variants
          </div>
          <div>
