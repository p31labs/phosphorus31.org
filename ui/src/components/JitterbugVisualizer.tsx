...
function JitterbugScene({
  ...
  useFrame(() => {
    jitterbugShaders.updateShaders(performance.now() * 0.001, entropy, playerPosition);
  });
  ...
  return (
    <>
      ...
      <ShaderMesh shaderName={currentShader} entropy={entropy} playerPosition={playerPosition} />
    ...
    >
      ...
      >
        ...
        <div style={{ marginBottom: '15px' }}>
          <label
            style={{ display: 'block', marginBottom: '5px', fontSize: '14px', color: '#ffffff' }}
          >
            Shader Effect:
          </label>
          <select
        ...
        <div style={{ marginBottom: '15px' }}>
          <label
            style={{ display: 'block', marginBottom: '8px', fontSize: '14px', color: '#ffffff' }}
          >
            Visual Effects:
          </label>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px' }}>
            <label
              style={{ display: 'flex', alignItems: 'center', fontSize: '12px', color: '#cccccc' }}
            >
              <input
                type="checkbox"
      ...
      >
        ...
        <div>
          Effects:{' '}
          {[showParticles && 'Particles', showOrbs && 'Orbs', showCascades && 'Cascades']
            .filter(Boolean)
            .join(', ') || 'None'}
        </div>
      ...
      >
        <strong>🎭 Jitterbug Controls:</strong>
        <br />
        • Select shaders to explore different entropy visualizations
        <br />
        • Adjust entropy level to see state transitions
        <br />
        • Toggle effects for mesmerizing combinations
        <br />
        • Enable auto-cycle for continuous shader evolution
        <br />
        • Go fullscreen for immersive quantum chaos experience
        <br />
        <span style={{ color: '#8b5cf6', fontStyle: 'italic' }}>
          <br />
          "Watch as mathematics dances into visual poetry..."
        </span>
      </div>
    </div>
  );
