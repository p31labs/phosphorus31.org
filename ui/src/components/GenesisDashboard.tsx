...
import { usePhenixStore } from '../stores/phenix.store';
...
function TetrahedronStatus({ nodes }: { nodes: MeshNode[] }) {
  ...
  return (
    <div className="relative h-48 bg-zinc-900/50 rounded-lg border border-zinc-700 overflow-hidden">
        ...
        {nodePositions.map((np, i) => {
          ...
          return (
            <g key={i} transform={`translate(${x}, ${y})`}>
              ...
              {/* Node circle */}
              <circle r="8" fill="black" stroke={color} strokeWidth="2" />

              {/* Node ID */}
              <text textAnchor="middle" dy="3" fill={color} fontSize="8" fontFamily="monospace">
      ...
      {/* Labels */}
      <div className="absolute bottom-2 left-2 text-xs text-zinc-500">K₄ TOPOLOGY</div>
  ...
  return (
    <div className="bg-zinc-900/50 rounded-lg border border-zinc-700 p-3">
      ...
      <svg className="w-full h-16" viewBox="0 0 100 100" preserveAspectRatio="none">
        ...
        {/* Current value dot */}
        {data.length > 0 && (
          <circle cx="100" cy={100 - (current / max) * 100} r="3" fill={color}>
...
function QuantumMetrics() {
  ...
  return (
    <div className="bg-zinc-900/50 rounded-lg border border-zinc-700 p-4">
      <div className="text-zinc-400 text-xs uppercase tracking-wider mb-3">QUANTUM STATE</div>
      ...
      <div className="text-center">
        ...
        <span className="text-zinc-500 text-xs ml-2">= {binaryValue}</span>
      </div>
...
export default function GenesisDashboard() {
  ...
  const meshNodes: MeshNode[] = [
    ...
    },
    {
      ...
      lastSeen: lastSync || 0,
      voltage: deviceState?.voltage || 5,
      spoons: deviceState?.spoons || 10,
      rssi: -52,
    },
    {
      id: 0x03,
    ...
    <div className="bg-[#0a0a0f] text-white p-6 rounded-xl border border-zinc-800 space-y-6">
      ...
      <div className="flex items-center justify-between border-b border-zinc-800 pb-4">
        <div>
          ...
          <p className="text-zinc-500 text-sm mt-1">Enterprise Mesh Control Interface</p>
        </div>

        <div className="text-right">
          <div
            className={`flex items-center gap-2 ${isConnected ? 'text-green-400' : 'text-zinc-500'}`}
          >
            <span
              className={`w-2 h-2 rounded-full ${isConnected ? 'bg-green-400 animate-pulse' : 'bg-zinc-600'}`}
            />
            {isConnected ? 'LINKED' : 'DISCONNECTED'}
          </div>
          <div className="text-zinc-600 text-xs mt-1 font-mono">v1.0.0 | K₄ Protocol</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          <PhiGauge value={voltage} max={10} label="VOLTAGE" color="#ef4444" icon="⚡" />

          <PhiGauge value={spoons} max={20} label="SPOONS" color="#06b6d4" icon="🥄" />

          <PhiGauge value={battery} max={100} label="BATTERY" color="#22c55e" icon="🔋" />
            ...
            {meshNodes.map((node) => (
              ...
              >
                ...
                <div className="text-right text-xs">
                  ...
                  <div className="text-zinc-600">{node.rssi} dBm</div>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Right Column - Graphs */}
        <div className="space-y-4">
          <TelemetryGraph data={voltageHistory} label="Voltage History" color="#ef4444" max={10} />

          <TelemetryGraph data={spoonsHistory} label="Spoons History" color="#06b6d4" max={20} />

          <TelemetryGraph data={heapHistory} label="Heap Memory (KB)" color="#a855f7" max={300} />
      ...
      <div className="flex items-center justify-between text-xs text-zinc-600 pt-4 border-t border-zinc-800">
        ...
        <div className="font-mono">{new Date().toISOString().split('T')[0]} · GENESIS GATE</div>
      </div>
    </div>
  );
}
