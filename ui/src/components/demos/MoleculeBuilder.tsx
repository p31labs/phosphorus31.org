...
const PRESETS: Record<
  string,
  { atoms: Omit<Atom, 'id'>[]; bonds: Omit<Bond, 'id'>[]; name: string }
> = {
  water: {
    name: 'Water (H₂O)',
    atoms: [
...
const BondLine: React.FC<{ bond: Bond; atoms: Atom[] }> = ({ bond, atoms }) => {
  ...
  const points = [new THREE.Vector3(...fromAtom.position), new THREE.Vector3(...toAtom.position)];

  return <Line points={points} color="#CCCCCC" lineWidth={bond.order} />;
};
    ...
    >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '16px', flex: 1 }}>
        ...
        >
          ...
          >
            <div style={{ fontSize: '11px', color: '#9CA3AF', marginBottom: '4px' }}>Formula</div>
            <div
              style={{
                fontSize: '16px',
                fontWeight: 600,
            ...
            <div style={{ fontSize: '10px', color: '#9CA3AF' }}>Mass: {totalMass} amu</div>
          </div>
        </div>
      ...
      >
        <p style={{ fontSize: '12px', color: '#C084FC', margin: 0 }}>
          ...
          enable "Create Bond" and click two atoms to connect them, drag atoms to move them in 3D
          space.
        </p>
      </div>
    </div>
