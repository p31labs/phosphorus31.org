...
export default function SonicShield() {
  ...
  const currentColor =
    activeMode === 'soundscape' ? currentSoundscape?.color : currentBinaural?.color;
  ...
  return (
    ...
    >
      ...
      >
        <button
          ...
          style={{
            ...
            border:
              activeMode === 'soundscape'
                ? `1px solid ${GOD_CONFIG.theme.border.accent}`
      ...
      {activeMode === 'soundscape' && (
        ...
        >
          {SOUNDSCAPES.map((soundscape) => (
            ...
            >
              <div style={{ fontSize: '2rem', marginBottom: '0.5rem' }}>{soundscape.icon}</div>
              <div style={{ fontWeight: 'bold', marginBottom: '0.25rem' }}>{soundscape.name}</div>
          ...
          {BINAURAL_PRESETS.map((preset) => (
            <button
              ...
              style={{
                padding: '1rem',
                background:
                  selectedBinaural === preset.id ? preset.color : GOD_CONFIG.theme.bg.secondary,
                border:
                  selectedBinaural === preset.id
                    ? `2px solid ${GOD_CONFIG.theme.border.accent}`
                    : `1px solid ${GOD_CONFIG.theme.border.default}`,
                borderRadius: '0.75rem',
                color: GOD_CONFIG.theme.text.primary,
      ...
      {activeMode === 'binaural' && (
        ...
        >
          🎧 <strong style={{ color: GOD_CONFIG.theme.text.primary }}>Headphones required</strong>{' '}
          for binaural beats to work. The effect comes from the difference between left and right
          channels.
        </div>
      )}
      ...
      >
        ...
        <button
          ...
          style={{
            ...
            fontSize: '2.5rem',
            cursor: 'pointer',
            boxShadow: isPlaying ? '0 0 30px rgba(239,68,68,0.5)' : '0 0 30px rgba(34,197,94,0.5)',
            transition: 'all 0.3s ease',
          }}
        >
          {isPlaying ? '⏹' : '▶'}
      ...
      >
        ...
        <div
          style={{ fontSize: '1.2rem', fontWeight: 'bold', color: GOD_CONFIG.theme.text.primary }}
        >
          {activeMode === 'soundscape'
      ...
      >
        ...
        {activeMode === 'soundscape' ? (
          <>
            Soundscapes use <strong>frequency layering</strong> and <strong>colored noise</strong>{' '}
            to create immersive audio environments. Brown noise emphasizes low frequencies
            (grounding), pink noise has equal energy per octave (natural), and white noise is flat
            (masking).
          </>
        ) : (
          <>
            Binaural beats work through <strong>frequency following response</strong>. When two
            slightly different tones are played in each ear, your brain perceives a third "beat" at
            the difference frequency. A 200Hz left + 210Hz right = 10Hz alpha wave entrainment.
          </>
        )}
      </div>
    </div>
