...
export default function WebContainerIDE() {
  ...
  // UI state
  const [activeTab, setActiveTab] = useState<'editor' | 'terminal' | 'preview' | 'packages'>(
    'editor'
  );
  ...
  useEffect(() => {
    ...
    const handleCommandOutput = (data: any) => {
      setCommandHistory((prev) =>
        prev.map((cmd) =>
          cmd.id === data.commandId ? { ...cmd, output: [...cmd.output, data.output] } : cmd
        )
      );
    };
    ...
    >
      ...
      >
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: sidebarCollapsed ? '8px' : '0' }}>
          {sidebarCollapsed ? (
            <div style={{ textAlign: 'center', padding: '10px 0' }}>📁</div>
          ) : (
            files.map((file) => (
              <div
                key={file.path}
            ...
            <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  ...
                  <span style={{ fontWeight: '500' }}>{selectedFile || 'No file selected'}</span>
                  {selectedFile && (
                    <span
                      style={{
                        fontSize: '12px',
              ...
              <textarea
                ...
                placeholder={
                  selectedFile ? undefined : 'Select a file from the sidebar to start editing...'
                }
                style={{
                  flex: 1,
                  padding: '20px',
              ...
              >
                {commandHistory.map((cmd) => (
                  <div key={cmd.id} style={{ marginBottom: '8px' }}>
                    <div style={{ color: '#0ff' }}>$ {cmd.command}</div>
                    {cmd.output.map((line, i) => (
                      <div key={i} style={{ color: cmd.status === 'error' ? '#f00' : '#0f0' }}>
                        {line}
                      </div>
                    ))}
                    {cmd.status === 'running' && <div style={{ color: '#ff0' }}>⏳ Running...</div>}
                  </div>
                ))}
          ...
          {activeTab === 'packages' && (
            <div style={{ height: '100%', padding: '20px', overflowY: 'auto' }}>
              ...
              <div>
                ...
                >
                  <div>
                    <strong>@noble/post-quantum:</strong> ^0.1.0 (ML-KEM-768)
                  </div>
                  <div>
                    <strong>express:</strong> ^4.18.2 (Web server)
                  </div>
                  <div>
                    <strong>ws:</strong> ^8.13.0 (WebSocket support)
                  </div>
                </div>
              </div>
              ...
              <div style={{ marginTop: '24px' }}>
                ...
                >
                  <p>
                    <strong>ML-KEM-768:</strong> Post-quantum key encapsulation providing security
                    against quantum computers.
                  </p>
                  <p>
                    <strong>Express:</strong> Fast, unopinionated web framework for Node.js.
                  </p>
                  <p>
                    <strong>WebSocket:</strong> Real-time communication protocol for live updates.
                  </p>
                </div>
              </div>
            </div>
