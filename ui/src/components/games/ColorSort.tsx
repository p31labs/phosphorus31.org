...
export const ColorSort: React.FC = () => {
  ...
  const [buckets, setBuckets] = useState<Bucket[]>(BUCKET_COLORS.map((b) => ({ ...b, items: [] })));
  ...
  const handleBucketClick = useCallback(
    (bucketIndex: number) => {
      ...
      if (bucket.color === selectedItem.color) {
        ...
        setBuckets((prev) =>
          prev.map((b, i) => (i === bucketIndex ? { ...b, items: [...b.items, selectedItem] } : b))
        );
  ...
  return (
    ...
    >
      ...
      >
        {!isGameActive
          ? isRoundComplete
            ? '✨ All sorted! Tap for another round.'
            : 'Tap an item, then tap its matching bucket.'
          : selectedItem
            ? 'Now tap the matching bucket!'
            : 'Tap a colored circle to pick it up.'}
      </p>
        ...
        {unsortedItems.map((item) => (
          <button
            ...
            style={{
              ...
              boxShadow: selectedItem?.id === item.id ? `0 0 20px ${item.color}` : 'none',
            }}
          />
        ))}
        {unsortedItems.length === 0 && !isRoundComplete && (
          <div style={{ color: '#6B7280', fontSize: '14px' }}>Tap "New Round" to start</div>
        )}
      </div>
