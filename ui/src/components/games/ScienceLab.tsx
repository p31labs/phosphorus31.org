...
function findReaction(a: string, b: string): Reaction['result'] | null {
  const reaction = REACTIONS.find(
    (r) =>
      (r.elements[0] === a && r.elements[1] === b) || (r.elements[0] === b && r.elements[1] === a)
  );
    ...
    >
      ...
      {showEasterEgg && (
        ...
        >
          ...
          <div style={{ fontSize: '16px', fontWeight: 700 }}>You found a secret!</div>
      ...
      >
        ...
        {selected.length > 0 && <span style={{ fontSize: '24px', color: '#9CA3AF' }}>+</span>}
        {selected[1] && (
          <span style={{ fontSize: '40px' }}>
      ...
      {result && (
        ...
        >
          ...
          <div style={{ fontSize: '13px', color: '#D1D5DB', lineHeight: 1.5 }}>{result.fact}</div>
        </div>
      )}
