...
export const PatternTap: React.FC = () => {
  ...
  const flashTile = useCallback((tileId: number, duration = 400) => {
    setTiles((prev) => prev.map((t) => (t.id === tileId ? { ...t, isLit: true } : t)));
    setTimeout(() => {
      setTiles((prev) => prev.map((t) => (t.id === tileId ? { ...t, isLit: false } : t)));
    }, duration);
  }, []);
        ...
        {tiles.map((tile) => (
          <button
            ...
            style={{
              ...
              borderRadius: '16px',
              border: 'none',
              background: tile.isLit ? tile.color : `${tile.color}30`,
              boxShadow: tile.isLit ? `0 0 30px ${tile.color}, 0 0 60px ${tile.color}50` : 'none',
