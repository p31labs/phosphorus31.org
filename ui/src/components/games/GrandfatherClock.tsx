    ...
    <group ref={groupRef} position={[0, 2, 0]}>
      ...
      <mesh position={[0, -1.5, 0]}>
        ...
        <meshStandardMaterial
          ...
          metalness={0.7}
          emissive={CosmicTheme.colors.cosmic}
      ...
      <mesh position={[0, -3, 0]}>
        ...
        <meshStandardMaterial
          ...
          metalness={0.9}
          roughness={0.1}
          emissive={CosmicTheme.colors.love}
  ...
  return (
    <group position={[0, 3, 0.1]}>
      ...
      <mesh rotation={[0, 0, hourAngle]}>
        ...
        <meshStandardMaterial
          color={CosmicTheme.colors.delta}
          emissive={CosmicTheme.colors.cosmic}
      ...
      <mesh rotation={[0, 0, secondAngle]}>
        ...
        <meshStandardMaterial
          ...
          emissive={CosmicTheme.colors.signal}
    ...
    <group position={[3, 1, 0]}>
      ...
      <mesh geometry={tubeGeometry}>
        <meshStandardMaterial
          ...
          transparent
          opacity={0.18}
          emissive={CosmicTheme.colors.delta}
          emissiveIntensity={0.2}
        />
      </mesh>
      <Text
  ...
  return (
    <>
      ...
      <Text
        ...
        color={CosmicTheme.colors.delta}
        outlineWidth={0.01}
        outlineColor={CosmicTheme.colors.cosmic}
      >
        VI
      </Text>
      <Text
      ...
      <Text
        position={[0, 5.2, 0.45]}
        fontSize={0.13}
        color={CosmicTheme.ratioOverlay.style.color}
        anchorX="center"
        anchorY="middle"
        outlineWidth={0.008}
        outlineColor={CosmicTheme.colors.love}
...
const GrandfatherClock: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const isCosmicAnniversary = COSMIC_MOMENTS.some(
      (moment) => now.getMonth() + 1 === moment.month && now.getDate() === moment.day
    );
    ...
    <div className="w-full h-full relative cosmic-clock-bg">
      ...
      <div className="absolute top-4 left-4 z-10">
        ...
        <p className="text-sm text-amber-300 max-w-md">
          Time is a spiral, not a line. The clock remembers every moment— from 1925 to 2026, the
          100-year cycle breathes.
        </p>
      </div>

      {/* Current time display */}
      <div className="absolute top-4 right-4 z-10 bg-amber-900/80 backdrop-blur-sm px-6 py-3 rounded-lg">
        <div className="text-2xl font-bold text-amber-100">{currentTime.toLocaleTimeString()}</div>
      ...
      {selectedMoment && (
        <div
          className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-20 cosmic-moment-details"
          data-color={selectedMoment.color}
        >
          <div className="text-center">
            <div className="text-6xl mb-3">{selectedMoment.emoji}</div>
            <h3
              className="text-2xl font-bold mb-2 cosmic-moment-details-label"
              data-color={selectedMoment.color}
            >
            ...
            <p className="text-amber-100 text-sm mb-4">{selectedMoment.description}</p>
            <button
      ...
      <div className="absolute bottom-28 right-4 z-10 max-w-xs text-right">
        <p className="text-amber-300 text-sm italic">
          "Time isn't linear. It's a spiral. A Jitterbug breathing like the cosmos itself."
        </p>
      ...
      {showCuckoo && (
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-30 pointer-events-none">
          ...
          <div className="text-2xl text-amber-200 text-center mt-2 font-bold">CUCKOO! CUCKOO!</div>
        </div>
      )}
    </div>
  );
