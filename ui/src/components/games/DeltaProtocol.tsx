...
const VPI_PHASES = ['Porous', 'Vacuum', 'Resin', 'Pressure', 'Cured'];

export const DeltaProtocol: React.FC = () => {
  const [gamePhase, setGamePhase] = useState<
    'intro' | 'building' | 'vpi' | 'abdication' | 'victory'
  >('intro');
  ...
  const claimNode = useCallback(
    (nodeId: string, faction: FactionType) => {
      setNodes((prev) => prev.map((n) => (n.id === nodeId ? { ...n, faction } : n)));
  ...
  const resistConvergence = useCallback(() => {
    ...
    if (vulnerableNodes.length > 0) {
      ...
      setNodes((prev) =>
        prev.map((n) => (n.id === target.id ? { ...n, stress: Math.min(100, n.stress + 30) } : n))
      );
  ...
  if (gamePhase === 'intro') {
    return (
      ...
      >
        ...
        >
          <h2 style={{ fontSize: '18px', color: '#10B981', marginBottom: '12px' }}>
            Your Mission:
          </h2>
          <ol
            style={{ color: '#D1D5DB', fontSize: '14px', lineHeight: '1.8', paddingLeft: '20px' }}
          >
        ...
        >
          <p style={{ fontSize: '13px', color: '#F59E0B', margin: 0 }}>
            <strong>Remember:</strong> Diversity creates stability. A Tetrahedron needs 4 DIFFERENT
            nodes. Connection without protocol is assimilation. Protocol without connection is
            isolation.
          </p>
        </div>
  ...
  if (gamePhase === 'victory') {
    return (
      ...
      >
        ...
        <h1 style={{ fontSize: '36px', color: '#10B981', marginBottom: '12px' }}>GREEN BOARD</h1>
        ...
        >
          <div style={{ background: '#1E293B', padding: '16px', borderRadius: '8px' }}>
            <div style={{ fontSize: '24px', color: '#10B981', fontWeight: 600 }}>{score}</div>
        ...
        >
          <p style={{ fontSize: '13px', color: '#10B981', margin: 0 }}>
            <strong>Achievement Unlocked:</strong> "The First Tetrahedron" - You've built your first
            headless mesh cluster. The protocol is now autonomous and immortal.
          </p>
        </div>
  ...
  return (
    ...
    >
      ...
      >
        <div>
          <h2 style={{ fontSize: '24px', color: '#F3F4F6', margin: 0 }}>🜂 Delta Protocol</h2>
        ...
        <div style={{ display: 'flex', gap: '12px' }}>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: '20px', color: '#10B981', fontWeight: 600 }}>{score}</div>
      ...
      {message && (
        ...
        >
          <p style={{ fontSize: '13px', color: '#D1D5DB', margin: 0 }}>{message}</p>
        </div>
      )}
              ...
              {node.faction === 'Delta' && (
                <>
                    ...
                    >
                      <div
                        style={{
                          ...
                          height: '100%',
                          background:
                            node.stress > 70 ? '#EF4444' : node.stress > 40 ? '#F59E0B' : '#10B981',
                          transition: 'all 0.3s',
                        }}
                      />
                    </div>
                  ...
                  <div style={{ fontSize: '10px' }}>
                    <div style={{ color: '#10B981', marginBottom: '2px' }}>✓ {info.strength}</div>
                    <div style={{ color: '#EF4444' }}>✗ {info.weakness}</div>
                  </div>
                </>
              )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px' }}>
        <button
          ...
          style={{
            padding: '12px',
            background:
              selectedNode && nodes.find((n) => n.id === selectedNode)?.vpiPhase !== 4
