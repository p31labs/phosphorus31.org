...
describe('GenreDetector', () => {
  describe('Physics (factual) detection', () => {
    test('factual statements with data → "physics"', () => {
      const content =
        'The payment was $500 on March 3rd. Deadline is Friday. Task status: complete.';
    ...
    test('deadline and action items → "physics"', () => {
      const content =
        'Deadline: Friday. Action items: Review document, verify data, confirm status';
      const result = detectGenre(content);
      expect(result.genre).toBe('physics');
    });
  });
  ...
  describe('Poetics (emotional) detection', () => {
    test('emotional appeals → "poetics"', () => {
      const content =
        "I can't believe you'd do this to us. I feel hurt and disappointed. I care about our relationship.";
      const result = detectGenre(content);
  ...
  describe('Genre confidence', () => {
    ...
    test('high confidence for clear poetics', () => {
      const content =
        'I feel hurt and sad. I love you and care about our relationship. Thank you for understanding';
      const result = detectGenre(content);
      if (result.genre === 'poetics') {
        expect(result.confidence).toBeGreaterThan(0.6);
      }
  ...
  describe('Score calculation', () => {
    ...
    test('scores are normalized by message length', () => {
      ...
      const long = detectGenre(
        'Deadline task status report data review complete submit verify check confirm update'
      );
    ...
    test('scores are clamped to 0-1 range', () => {
      const result = detectGenre(
        'Deadline task status report data review complete submit verify check confirm update action item list need required must should'
      );
  ...
  describe('Indicators', () => {
      ...
      if (result.genre === 'physics') {
        expect(
          result.indicators.some(
            (i) => i.toLowerCase().includes('factual') || i.toLowerCase().includes('task')
          )
        ).toBe(true);
      }
    });
    ...
    test('poetics genre includes poetics indicator', () => {
      ...
      if (result.genre === 'poetics') {
        expect(
          result.indicators.some(
            (i) => i.toLowerCase().includes('emotional') || i.toLowerCase().includes('relational')
          )
        ).toBe(true);
      }
    });
  ...
  describe('Real-world examples', () => {
    test('business email → physics', () => {
      const content =
        'Please review the attached document and provide feedback by end of week. Status update required.';
      const result = detectGenre(content);
      expect(result.genre).toBe('physics');
    });
    ...
    test('personal message → poetics', () => {
      const content =
        "I'm sorry I hurt your feelings. I care about you and want to make things right";
      const result = detectGenre(content);
      expect(result.genre).toBe('poetics');
    });
