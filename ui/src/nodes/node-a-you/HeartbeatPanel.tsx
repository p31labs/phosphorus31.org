...
export function HeartbeatPanel() {
  ...
  const [userName, setUserName] = useState(localStorage.getItem('heartbeat-user-name') || '');
  const [showDailyCheckIn, setShowDailyCheckIn] = useState(false);
  const [showHistory, setShowHistory] = useState(false);

  const statusConfig = GOD_CONFIG.heartbeat.statuses[currentStatus];
  const intervalConfig = checkInInterval
    ? GOD_CONFIG.heartbeat.checkInIntervals.find((i) => i.id === checkInInterval.id)
    : null;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      {isDeadManActive && intervalConfig && (intervalConfig.ms ?? 0) > 0 && (
        <div
          style={{
            padding: 16,
            backgroundColor: GOD_CONFIG.theme.bg.secondary,
        ...
        >
          ...
          <div style={{ marginTop: 12 }}>
            <select
              value={checkInInterval?.id || ''}
              onChange={(e) => {
                const interval = GOD_CONFIG.heartbeat.checkInIntervals.find((i) => i.id === e.target.value);
                if (interval) {
                  setCheckInInterval({
                    id: interval.id,
                    label: interval.label,
                    ms: interval.ms ?? interval.minutes * 60 * 1000,
