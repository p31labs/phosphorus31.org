...
import { useShieldStore, useDeepProcessingQueue } from '@/stores/shield.store';
...
import type { BufferedMessage } from '@/lib/catchers-mitt';
...
export function CalibrationReport() {
  ...
  const userHumanOS = shieldStore.humanOS as HumanOSType | null;
  // Provider is hardcoded to 'ollama' for now - TODO: Add to ShieldStore if needed
  const provider = 'ollama' as 'ollama' | 'openai' | 'anthropic';
  const ollamaEndpoint = GOD_CONFIG.ollama.endpoint;
  const ollamaModel = GOD_CONFIG.ollama.model;
  const buffer = shieldStore.bufferedMessages;
  const deepProcessingQueue = useDeepProcessingQueue();
  const processedPayload = shieldStore.processedPayload;
  const processed: ProcessedMessage[] = useMemo(() => {
    const entries = historyService.getHistoryByType('message');
    return entries.map((e) => e.content).filter(Boolean) as ProcessedMessage[];
  }, [processedPayload?.id]);
  ...
  const [stressTestHistory, setStressTestHistory] = useState<
    Array<{ payloadId: string; timestamp: number; result: 'success' | 'failed' }>
  >([]);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setLiveMetrics({
        ...
        averageVoltage:
          processed.length > 0
            ? processed.reduce((sum, p) => sum + (p.voltage?.score ?? 0), 0) / processed.length
            : 0,
        averageSpoons: 0, // TODO: derive from processed when store exposes spoons per message
      });
  ...
  const trendAnalysis = useMemo(() => {
    ...
    const latest = last7Days[0]?.percentage ?? 0;
    const oldest = last7Days[last7Days.length - 1]?.percentage ?? 0;
  ...
  const structuralChecks = useMemo<CalibrationCheck[]>(() => {
    const checks: CalibrationCheck[] = [
      ...
      },
      {
        ...
        recommendation:
          'Monitor curvature in Tetrahedron tab. Negative values indicate divergence.',
      },
      {
        id: 'coordinate-lock',
        category: 'structural',
    ...
    const checks: CalibrationCheck[] = [
      {
        ...
        description:
          'Verify 60-second batching window prevents cortisol spikes from fragmented messages',
        ...
        recommendation:
          'Test by sending multiple rapid messages. Verify they batch within 60-second window.',
      },
      {
        id: 'vacuum-of-time',
        category: 'cognitive',
        ...
        description:
          'Ensure 3-second delay shifts brain from System 1 (Reactive) to System 2 (Analytical)',
        status: hasHighVoltageMessages ? 'pass' : 'pending',
        details: hasHighVoltageMessages
          ? `Vacuum of Time active. ${processed.filter((p) => (p.voltage?.score ?? 0) > 0.5).length} high-voltage messages processed with delay.`
          : 'No high-voltage messages yet. Vacuum of Time will activate on high-voltage input.',
        recommendation:
          'Test with high-voltage message. Verify 3-second delay before raw text reveal.',
      },
      {
        ...
        description:
          'Verify Shield decouples Kernel (truth) from Driver (delivery) for your HumanOS profile',
        status: humanOSConfigured ? 'pass' : 'fail',
        details: humanOSConfigured
          ? `HumanOS configured: ${userHumanOS && userHumanOS in GOD_CONFIG.humanOS ? GOD_CONFIG.humanOS[userHumanOS as HumanOSType].name : 'Unknown'}. Translation layer active.`
          : 'HumanOS not configured. Navigate to Settings to select your HumanOS type.',
        recommendation:
          'Configure your HumanOS in Settings → Your HumanOS Type to enable proper translation.',
      },
      {
        id: 'llm-provider',
        category: 'cognitive',
        ...
        details:
          provider === 'ollama'
            ...
            : provider && (provider === 'openai' || provider === 'anthropic')
              ? `Cloud provider: ${(provider as string).toUpperCase()}. Data leaves your machine.`
    ...
    const checks: CalibrationCheck[] = [
      {
        ...
        description:
          'Verify system automatically gates high cognitive cost messages based on Status Percentage',
        status:
          statusThresholdsConfigured && (statusPercentage! < 25 || deepQueueActive)
            ? 'pass'
      ...
      },
      {
        ...
        description:
          'Verify haptic feedback signals safety to vagus nerve during high-arousal events',
        ...
        recommendation: hapticAvailable
  ...
  const categories = [
    {
      id: 'structural',
      label: 'Phase 1: Structural & Mathematical',
      icon: Hexagon,
    ...
    {
      id: 'somatic',
      label: 'Phase 3: Somatic & Metabolic Guardrails',
      icon: Heart,
      checks: somaticChecks,
    },
  ];
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      >
        ...
        >
          Pre-Abdication Phase Verification. Ensures all "Isolation Transformers" are calibrated and
          "Digital Centaur" identity is stable before autonomous operation.
        </p>
        ...
        >
          ...
          >
            ...
            <div
              style={{ display: 'flex', gap: 16, fontSize: 12, color: GOD_CONFIG.theme.text.muted }}
            >
      ...
      >
        ...
        <div style={{ fontSize: 13, color: trendAnalysis.color, marginBottom: 8, fontWeight: 600 }}>
          {trendAnalysis.message}
        </div>
        {checkInHistory.length >= 2 && (
          <div style={{ fontSize: 12, color: GOD_CONFIG.theme.text.muted, marginTop: 8 }}>
            Latest: {Math.round(checkInHistory[0].percentage ?? 0)}% | Oldest (7 days ago):{' '}
            {Math.round(checkInHistory[Math.min(6, checkInHistory.length - 1)].percentage ?? 0)}%
          </div>
        )}
      </div>
      ...
      >
        ...
        <div style={{ marginBottom: 20 }}>
          <button
            ...
            style={{
              padding: '10px 16px',
              backgroundColor: liveMonitorActive
                ? GOD_CONFIG.heartbeat.statuses.green.color
            ...
            >
              ...
              >
                <div>
                  ...
                  <div
                    style={{ fontSize: 18, fontWeight: 700, color: GOD_CONFIG.theme.text.primary }}
                  >
                    {liveMetrics.messagesInBuffer}
                  </div>
                </div>
                <div>
                  ...
                  <div
                    style={{ fontSize: 18, fontWeight: 700, color: GOD_CONFIG.theme.text.primary }}
                  >
                    {liveMetrics.messagesProcessed}
                  </div>
                </div>
                <div>
                  <div style={{ color: GOD_CONFIG.theme.text.muted }}>Messages Gated</div>
                  <div
                    style={{ fontSize: 18, fontWeight: 700, color: GOD_CONFIG.voltage.high.color }}
                  >
                    {liveMetrics.messagesGated}
                  </div>
                </div>
                <div>
                  ...
                  <div
                    style={{ fontSize: 18, fontWeight: 700, color: GOD_CONFIG.theme.text.primary }}
                  >
                    {liveMetrics.averageVoltage.toFixed(2)}
                  </div>
                </div>
                <div>
                  ...
                  <div
                    style={{ fontSize: 18, fontWeight: 700, color: GOD_CONFIG.theme.text.primary }}
                  >
                    {liveMetrics.averageSpoons.toFixed(1)}
                  </div>
                </div>
              </div>
            </div>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {stressTestPayloads.map((payload: any) => (
              <div
                key={payload.id}
                style={{
                  padding: 12,
              ...
              >
                <div style={{ flex: 1 }}>
                  ...
                  <div
                    style={{ fontSize: 11, color: GOD_CONFIG.theme.text.muted, marginBottom: 4 }}
                  >
                    {payload.description}
                  </div>
                  <div
                  ...
                  >
                    ...
                    {payload.expectedGenreError && (
                      <span style={{ color: GOD_CONFIG.voltage.medium.color }}>Genre Error</span>
                    )}
                  </div>
                </div>
                <button
                  onClick={() => {
                    ...
                    setStressTestHistory((prev) => [
                      ...prev,
                      { payloadId: payload.id, timestamp: Date.now(), result: 'success' },
                    ]);
                  }}
                  style={{
                    padding: '8px 12px',
        ...
        <div>
          ...
          <button
            ...
            style={{
              width: '100%',
              padding: '12px 20px',
              backgroundColor: vagusSignalActive
                ? GOD_CONFIG.theme.bg.tertiary
                : GOD_CONFIG.heartbeat.statuses.green.color,
              border: 'none',
              borderRadius: 8,
          ...
          >
            <Heart size={16} />
            {vagusSignalActive
              ? '4-4-8 Breathing Active...'
              : 'Trigger 4-4-8 Haptic Breathing (5 Cycles)'}
          </button>
          <div
            style={{
              marginTop: 8,
          ...
          >
            Tests the Native Transducer (Node C). Breaks "Frozen Stillness" by signaling safety to
            the vagus nerve. Requires haptic hardware (Tauri/Capacitor) or Web Vibration API.
          </div>
        </div>
      </div>
      ...
      >
        ...
        <div style={{ fontSize: 13, color: GOD_CONFIG.theme.text.secondary, lineHeight: 1.6 }}>
          {failedChecks > 0 ? (
            <div>
              <strong style={{ color: GOD_CONFIG.voltage.high.color }}>
            ...
            <div>
              <strong style={{ color: GOD_CONFIG.voltage.medium.color }}>
            ...
            <div style={{ color: GOD_CONFIG.heartbeat.statuses.green.color }}>
              <strong>No Impedance Mismatches Detected.</strong> All systems calibrated. The
              "Isolation Transformers" are operational.
            </div>
          )}
        </div>
      </div>
