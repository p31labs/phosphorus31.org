...
import HumanOSBadge from '@/components/HumanOSBadge';
...
export function ResponseComposer({ targetHumanOS }: ResponseComposerProps) {
  ...
  const [selectedOS, setSelectedOS] = useState<HumanOSType>(targetHumanOS || 'navigator');
  // API configuration - can be moved to store if needed
  ...
  const humanOSOptions = Object.entries(GOD_CONFIG.humanOS) as [
    HumanOSType,
    (typeof GOD_CONFIG.humanOS)[HumanOSType],
  ][];
          ...
          {humanOSOptions.map(([key, os]) => (
            <button
              ...
              style={{
                padding: '6px 12px',
                backgroundColor:
                  selectedOS === key ? `${os.color}20` : GOD_CONFIG.theme.bg.tertiary,
                border: `1px solid ${
                  selectedOS === key ? os.color : GOD_CONFIG.theme.border.default
                }`,
                borderRadius: 6,
                color: selectedOS === key ? os.color : GOD_CONFIG.theme.text.muted,
                fontSize: 11,
                fontFamily: GOD_CONFIG.typography.fontFamily.display,
                cursor: 'pointer',
                display: 'flex',
...
async function sanitizeWithOllama(
  ...
  const prompt = (GOD_CONFIG.prompts.sanitizeResponse || 'Sanitize this message for {humanOS}: {draft}')
    .replace('{humanOS}', `${os.name} (${os.coreImperative})`)
    .replace('{draft}', draft);

  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), GOD_CONFIG.ollama.timeout || 30000);
