...
export function MessageInput() {
  ...
  const submitMessage = useShieldStore((s) => s.submitMessage);

  const handleSubmit = () => {
    if (!message.trim()) return;
    submitMessage({
      id: `msg_${Date.now()}`,
      source: (source.trim() || 'manual') as 'email' | 'text' | 'chat' | 'manual' | 'mesh',
      sender: 'user',
    ...
    >
      ...
      <button
        ...
        style={{
          ...
          border: 'none',
          borderRadius: 8,
          color: message.trim() ? '#fff' : GOD_CONFIG.theme.text.muted,
          fontSize: 14,
          fontWeight: 600,
          fontFamily: GOD_CONFIG.typography.fontFamily.display,
      ...
      >
        💡 <strong>Pro Tip:</strong> Copy messages without reading them. The Shield will process and
        summarize, letting you engage your prefrontal cortex before your amygdala hijacks the
        response.
      </div>
    </div>
  );
