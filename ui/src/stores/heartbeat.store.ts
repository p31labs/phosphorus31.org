  ...
  devtools(
    persist(
      (set, get) => ({
        ...
        // Extended state for HeartbeatPanel
        currentStatus: 'green' as HeartbeatStatus,
        ...
        updateSpoons: (delta: number) => {
          ...
          const deepProcessingLock = heartbeatPercent < (HeartbeatConfig.lockoutThreshold ?? 20);
        ...
        setStatus: (status: HeartbeatStatus) => {
        ...
        setEscalationConfig: (config) => {
          set({ escalationConfig: config });
        },
        exportLog: () => {
          return JSON.stringify(get(), null, 2);
        },
        exportCheckInHistory: () => {
          return JSON.stringify(get().checkInHistory, null, 2);
        },
        getTodayCheckIn: () => {
          const today = new Date().toISOString().split('T')[0];
          return get().checkInHistory.find((c) => c.id.startsWith(today)) || null;
        },
        initializeMesh: () => {
          // Stub implementation
        },
...
export const useHeartbeatPercent = () =>
  useHeartbeatStore((state) => state.operator.heartbeatPercent);
...
export const useStressIndicators = () =>
  useHeartbeatStore((state) => state.operator.stressIndicators);
...
export const useHeartbeatColor = () => {
  ...
  return HeartbeatConfig.colors?.[status] ?? '#2ecc71';
};
