...
### 1. **WASM/WASI Sandbox** (Inner Hull) - Placeholder
- ⏳ Compile modules to WebAssembly (placeholder - structure ready)
- ⏳ Strict capability-based access control (design complete)
- ⏳ Memory-safe execution (requires WASM runtime)
- **Note**: Current implementation uses iframe sandbox (Outer Hull) only. WASM compilation pipeline is planned but not yet implemented.
...
### 3. **IPFS/ENS Integration** (Placeholder)
- ⏳ Decentralized module registry (structure complete, requires real IPFS service)
- ⏳ Content-addressed storage (CIDs) (simulated)
- ⏳ Human-readable names (ENS) (simulated)
- **Note**: Current implementation uses simulated CIDs. Real IPFS/ENS integration requires:
  - IPFS client (ipfs-http-client) or pinning service (Pinata, Web3.Storage)
  - ENS resolver contract or ENS.xyz service
  - Ethereum wallet for ENS registration
