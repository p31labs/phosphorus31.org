  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setLiveMetrics({
        ...
        averageVoltage:
          processed.length > 0
            ? processed.reduce((sum, p) => sum + p.voltage, 0) / processed.length
            : 0,
        averageSpoons:
          processed.length > 0
            ? processed.reduce((sum, p) => sum + p.spoons, 0) / processed.length
            : 0,
      });
      ...
      >
        ...
        {checkInHistory.length >= 2 && (
          <div style={{ fontSize: 12, color: GOD_CONFIG.theme.text.muted, marginTop: 8 }}>
            Latest: {Math.round(checkInHistory[0].percentage)}% | Oldest (7 days ago):{' '}
            {Math.round(checkInHistory[Math.min(6, checkInHistory.length - 1)].percentage)}%
          </div>
        )}
      </div>
