...
const TOOLS: ToolMeta[] = [
  {
    ...
    icon: '🌀',
    description: 'Infinite zoom into the Mandelbrot set',
  },
    ...
    <div>
      ...
      >
        ...
        <select
          value={colorScheme}
          onChange={(e) => {
            setColorScheme(e.target.value as typeof colorScheme);
          }}
      ...
      <p style={{ textAlign: 'center', fontSize: '0.8rem', opacity: 0.6, marginTop: '0.5rem' }}>
        Click to zoom in. Zoom: {zoom.toFixed(0)}x | Center: ({centerX.toFixed(4)},{' '}
        {centerY.toFixed(4)})
      </p>
    </div>
  );
        ...
        <div style={{ textAlign: 'center', marginTop: '0.5rem' }}>
          ...
          {clickCount === 49 && (
            <button
              ...
              style={{ ...btnStyle, marginLeft: '1rem', fontSize: '0.8rem' }}
            >
              Reset
            </button>
          )}
    ...
    <div>
      ...
      >
        <input
          ...
          value={searchTerm}
          onChange={(e) => {
            setSearchTerm(e.target.value.replace(/[^0-9]/g, ''));
          }}
        ...
        >
          ...
          <div style={{ maxHeight: '150px', overflowY: 'auto', marginTop: '0.5rem' }}>
            {results.slice(0, 10).map((r, i) => (
              <div
                key={i}
                style={{ fontFamily: 'monospace', fontSize: '0.9rem', margin: '0.25rem 0' }}
              >
                Position {r.position}: ...{r.context.replace(searchTerm, `[${searchTerm}]`)}...
              </div>
            ))}
            {results.length > 10 && (
              <div style={{ opacity: 0.6 }}>...and {results.length - 10} more</div>
            )}
          </div>
        </div>
      )}
      ...
      <div style={{ marginTop: '1rem' }}>
        <h4 style={{ textAlign: 'center', marginBottom: '0.5rem' }}>
          Digit Frequency (first 500 digits)
        </h4>
...
function MorseCode() {
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: '1rem' }}>
        <label style={{ display: 'block', marginBottom: '0.25rem', opacity: 0.7 }}>
          Morse Code:
        </label>
        <input
          type="text"
      ...
      {results.length > 0 && (
        ...
        >
          ...
          <button
            ...
            style={{ ...btnStyle, width: '100%', marginTop: '0.5rem' }}
          >
            Clear
          </button>
        </div>
  ...
  return (
    ...
    >
        ...
        {TOOLS.map((tool) => (
          <button
            ...
            style={{
              ...
              border:
                activeTool === tool.id
                  ? `2px solid ${GOD_CONFIG.theme.text.accent}`
      ...
      >
        ...
        <strong style={{ color: GOD_CONFIG.theme.text.primary }}>{currentTool?.name}</strong>:{' '}
