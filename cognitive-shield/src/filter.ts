...
export class MessageFilter {
  ...
  filter(message: QueuedMessage): FilterResult {
    ...
    const urgentKeywords = ['emergency', 'urgent', 'help', 'danger', 'critical'];
    const hasUrgentKeyword = urgentKeywords.some(keyword => content.includes(keyword));

    // Check for high voltage indicators
    const highVoltageKeywords = ['overwhelmed', 'shutdown', 'meltdown', 'overstimulated'];
    ...
    if (hasUrgentKeyword || hasHighVoltage) {
      priority = 'urgent';
      reason = 'Urgent keyword or high voltage detected';
